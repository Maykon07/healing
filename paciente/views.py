...
# home da página apos login
@login_required
def home(request):
    if request.method == "GET":
        # filtrando as escolhas do user
        medico_filtrar = request.GET.get('medico')
        especialidades_filtrar = request.GET.getlist('especialidades')

        # recuperando dados de todos os médicos
...
# verificação das consultas marcadas
@login_required
def minhas_consultas(request):
    # verificando se o user aplicou filtros
    especialidade_filtrar = request.GET.get('especialidade')
    ...
    if especialidade_filtrar:
        minhas_consultas = minhas_consultas.filter(data_aberta__user__dadosmedico__especialidade__id=especialidade_filtrar)
